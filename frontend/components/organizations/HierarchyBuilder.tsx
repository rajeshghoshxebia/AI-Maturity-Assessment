    ...
    <div>
      ...
      >
        ...
        {editing ? (
          ...
          <span
            className={cn("flex-1 min-w-0 text-sm text-grey-800 truncate", !readOnly && "cursor-pointer")}
            onDoubleClick={() => { if (!readOnly) { setDraft(unit.name); setEditing(true); } }}
          >
            {unit.name}
          </span>
        )}
        ...
        <select
          ...
          disabled={readOnly}
          className="text-xs text-grey-500 border border-grey-200 rounded px-1.5 py-0.5 focus:outline-none focus:border-velvet shrink-0 disabled:opacity-70"
        >
          {TYPE_OPTIONS.map((o) => (
  ...
  return (
    <div className="border border-grey-200 rounded-lg bg-white">
      ...
      <p className="px-4 py-2 text-xs text-grey-400 border-t border-grey-100 rounded-b-lg bg-white">
        {readOnly
          ? "Read-only view of the organisation hierarchy."
